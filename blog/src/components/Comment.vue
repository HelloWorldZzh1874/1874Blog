...
export default {
  ...
  methods: {
    ...
    checkReplies(index, item) {
      getReplay(item.id, 1).then(res => {
        this.$refs.check[index].style.display = "none";
        item.replyDTOList = res.data.data;
        //超过1页才显示分页
    ...
    changeReplyCurrent(current, index, commentId) {
      // 查看下一页回复
      getReplay(commentId, current).then(res => {
    ...
    listComments() {
      ...
      getArticleComments(articleId, this.current).then(res => {
    ...
    insertComment() {
      ...
      if (this.commentContent.trim() === "") {
        this.$toast({ type: "warnning", message: "评论不能为空" });
        return false;
      }
      //解析表情
      let reg = /\[.+?\]/g;
      ...
      const path = this.$route.path;
      const arr = path.split("/");
      let articleId = arr[arr.length - 1];
      if(articleId === "links"){
        articleId = -1;
      }
      let comment = {
    ...
    like(comment) {
      ...
      saveCommentsLike(param).then(res => {
        if (res.code === "200") {
          //判断是否点赞
    ...
    reloadReply(index) {
      ...
      getReplay(this.commentList[index].id, current).then(res => {
